...
int main()
{
	...
	sf::Text Note;
	Note.setString("Note: All calculated total distances include the distance added by changing elevation.");
	Note.setFont(arial);
	Note.setFillColor(sf::Color::Black);
	Note.setCharacterSize(20);
	Note.setPosition(100, 400);
	...
	Button calcBtn("Calculate", { 250,85 }, 60, sf::Color::White, sf::Color::Black);
	...
	calcBtn.setPosition({350,150});
	

	Button Solving("Solving...", { 350,85 }, 60, sf::Color::Green, sf::Color::Black);
	 //(textbox1.text.str().length() + 1) * 10
	Solving.setFont(arial);
	Solving.setPosition({350,150});


	Button SolvingD("Solving Dijkstra...", {500,85 }, 60, sf::Color::Red, sf::Color::Black);
	//(textbox1.text.str().length() + 1) * 10
	SolvingD.setFont(arial);
	SolvingD.setPosition({ 200,150 });

	Button SolvingB("Solving Bellman Ford...", { 575,85 }, 60, sf::Color::Yellow, sf::Color::Black);
	//(textbox1.text.str().length() + 1) * 10
	...
	resultBellman.setPosition(75, 250);
	...
	resultDijkstra.setPosition(500, 250);
			...
			while (window.pollEvent(anEvent)) {
				...
				case sf::Event::MouseButtonReleased:
					if (calcBtn.isMouseOver(window)) {
						...
						ansBellman += "Stops: \n";
						for (int i = 0; i < pathBell.size()-1; i++)
						...
						ansDijkstra += "Stops: \n";
						for (int i = 0; i < pathDijk.size() - 1; i++)
